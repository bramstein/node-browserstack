...
TestClient.API_BROWSERS_URL = TestClient.API_URL + 'browsers/';

TestClient.prototype.createWorker = function (options, callback) {
  this.request.post({
    url: BrowserStackTestClient.API_WORKER_URL.expand({}),
...
TestClient.prototype.getWorkers = function (callback) {
  this.request.get({
    url: BrowserStackTestClient.API_WORKERS_URL
  }, parseResponse(callback));
};

TestClient.prototype.getBrowsers = function (callback) {
  this.request.get({
    url: BrowserStackTestClient.API_BROWSERS_URL
  }, parseResponse(callback));
};
